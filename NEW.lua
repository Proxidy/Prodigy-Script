...
if not getgenv().StingrayLoaded then 
    ...
    -- Damn monkeys go AEEHOIIII, add to this table if you feel like a boss is running too much 
    ...
    local function Encode(data) 
        -- Pass data through url, safety encoding 
        ...
        task.spawn(function() 
            while Mobs:FindFirstChild(Boss) do 
                ...
                if Limiter % 240 == 0 then 
                    task.wait(5) 
                    -- Spam of ember insects can cause high pings, may cause disconnections and crashes 
                end 
                task.wait() 
            end 
        end) 
    ...
    -- Could have used WaitForChild here, but I felt it feels cursed not assigning WaitForChild to a variable, then I don't want an unused variable... 
    local Items = "| " 
    ...
    local Sent,Error = pcall(function() 
        if getgenv().Webhook then 
            ...
            local embed = { 
                ...
                ["description"] = "Script executed by " .. LocalPlayer.Name, 
                ["color"] = 16711680, 
                ["footer"] = { 
                    ["text"] = Executor .. " | " .. tostring(ChestsCollected) .. " chests collected" 
                }, 
            } 
            local data = { 
                ["embeds"] = {embed} 
